...
def load_data(AUDIO_MAX_LENGTH, TEXT_MAX_LENGTH, langs=['en','zh'],
    ...
    audio_transcript_pair_list = {'train':[], 'valid':[], 'test':[], 'test_normal':[]}
    for lang in langs:
        print('lang',lang)
        for split in audio_transcript_pair_list:
            if lang == 'en':
                txt_fn = os.path.join('data/wtimit/{}/{}.wrd'.format(split,split))
                tsv_fn = os.path.join('data/wtimit/{}/{}.tsv'.format(split,split))
            elif lang == 'zh':
                txt_fn = os.path.join('data/AISHELL6-Whisper/{}/{}.wrd'.format(split,split))
                tsv_fn = os.path.join('data/AISHELL6-Whisper/{}/{}.tsv'.format(split,split))
            ...
            with open(tsv_fn) as tsv:
                with open(txt_fn) as txt:
                    ...
                    if lang == 'en':
                        ...
                        wav_fns = [(audio.strip().split('\t')[2],  int(audio.strip().split('\t')[-2]), txt.strip(), 
                                    len(txt.strip()), int(audio.strip().split('\t')[-1])) for audio, txt in zip(audio_lns, txt_lns)]
...
class WhisperDataCollatorWhithPaddingNormalWhisper:
    def __call__(self, features):
        input_ids, labels, dec_input_ids, input_ids_normal,labels_normal,dec_input_ids_normal = [], [], [], [],[],[]
        for f in features:
            input_ids.append(f["input_ids"])
            labels.append(f["labels"])
            dec_input_ids.append(f["dec_input_ids"])
            input_ids_normal.append(f["input_ids_normal"])
            labels_normal.append(f["labels_normal"])
            dec_input_ids_normal.append(f["dec_input_ids_normal"])

        audio_lengths = [audio.shape[1] for audio in input_ids]
        audio_lengths2 = [audio.shape[1] for audio in input_ids_normal]
        max_audio_len =  max(max(audio_lengths),max(audio_lengths2))
        input_ids = [np.pad(audio, ((0, 0), (0, max_audio_len - audio_len)), 'constant', constant_values=0) for audio, audio_len in zip(input_ids, audio_lengths)]  
        input_ids_normal = [np.pad(audio, ((0, 0), (0, max_audio_len - audio_len)), 'constant', constant_values=0) for audio, audio_len in zip(input_ids_normal, audio_lengths2)]
        label_lengths = [len(lab) for lab in labels]
        dec_input_ids_length = [len(e) for e in dec_input_ids]
        # max_label_len = max(label_lengths+dec_input_ids_length) # seems redundant
        
        label_lengths_normal = [len(lab) for lab in labels_normal]
        dec_input_ids_length_normal = [len(e) for e in dec_input_ids_normal]
        
        max_label_len = max(max(max(label_lengths),max(dec_input_ids_length)),max(max(label_lengths_normal),max(dec_input_ids_length_normal))) # seems redundant
        ...
        # pad the labels with -100 (dummy, ignore index in cross-entropy), pad the dec_input_ids with eot
        labels_normal = [np.pad(lab, (0, max_label_len - lab_len), 'constant', constant_values=-100) for lab, lab_len in zip(labels_normal, label_lengths_normal)]
        dec_input_ids_normal = [np.pad(e, (0, max_label_len - e_len), 'constant', constant_values=50257) for e, e_len in zip(dec_input_ids_normal, dec_input_ids_length_normal)] # 50257 is eot token id
        batch = {
            "input_ids": input_ids,
            "labels": labels,
            "dec_input_ids": dec_input_ids,
            "input_ids_normal": input_ids_normal,
            "labels_normal": labels_normal,
            "dec_input_ids_normal": dec_input_ids_normal,
        }
        # Print shapes of all elements
        # for i in input_ids_normal:
        #     print(np.array(i).ndim)
...
class WhisperVideoCollatorWithPaddingNormalWhisper:
    def __call__(self, features):
        input_ids, labels, dec_input_ids, video, input_ids_normal,labels_normal,dec_input_ids_normal, video_normal= [], [], [], [], [],[],[],[]
        for f in features:
            input_ids.append(f["input_ids"])
            labels.append(f["labels"])
            dec_input_ids.append(f["dec_input_ids"])
            ...
            labels_normal.append(f["labels_normal"])
            dec_input_ids_normal.append(f["dec_input_ids_normal"])
            video_normal.append(f["video_normal"])
        ...
        # 0 pad the videos
        video_lengths = [len(vid) for vid in video]
        video_lengths2 = [len(vid) for vid in video_normal]
        max_video_len = max(max(video_lengths),max(video_lengths2))
        video = [np.pad(vid, ((0, max_video_len - vid_len), (0, 0), (0, 0), (0, 0)), 'constant', constant_values=0) for vid, vid_len in zip(video, video_lengths)]
        video_normal = [np.pad(vid, ((0, max_video_len - vid_len), (0, 0), (0, 0), (0, 0)), 'constant', constant_values=0) for vid, vid_len in zip(video_normal, video_lengths2)]
        padding_mask = create_padding_mask(max_video_len, [max_video_len - vid_len for vid_len in video_lengths])
        padding_mask_normal = create_padding_mask(max_video_len, [max_video_len - vid_len for vid_len in video_lengths2])
        batch = {
            "input_ids": input_ids,
            "labels": labels,
            "dec_input_ids": dec_input_ids,
            ...
            "padding_mask_normal": padding_mask_normal,
        }

        batch = {k: torch.tensor(np.array(v), requires_grad=False) for k, v in batch.items()}
        batch['video'] = batch['video'].permute((0, 4, 1, 2, 3)).contiguous() # [B, T, H, W, C] -> [B, C, T, H, W]
        batch['video_normal'] = batch['video_normal'].permute((0, 4, 1, 2, 3)).contiguous() # [B, T, H, W, C] -> [B, C, T, H, W]
...
def whisper_optimizer(model, cfg, t_total, video=True):
    ...
    print('cfg.learning_rate',cfg.learning_rate)
